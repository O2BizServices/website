...
function loadUsers() {
    $.ajax({
        url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/user/list',
        type: 'post',
		headers: {
...
function loadAllScreen(){
	 const SERVER_URL = getServerUrl();
    $.ajax({
        url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/screen/listScreen',
        type: 'post',
		headers: {
                'token': localStorage.getItem("token"),
        },
...
function loadAllRoles(reqtype){
	 const SERVER_URL = getServerUrl();
    $.ajax({
        url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/role/listRole',
        type: 'post',
		headers: {
                'token': localStorage.getItem("token"),
        },
...
function viewRoleUser(obj){
	var map = {};
		...
		$.ajax({
				url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/role/rolewiseuser',
				type: 'post',
				headers: {
                'token': localStorage.getItem("token"),
				},
...
function deleteRole(obj){
	if(confirm("Are you sure you want to delete this record?")){
		...
		$.ajax({
				url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/role/deleteRole',
				type: 'post',
				headers: {
                'token': localStorage.getItem("token"),
				},
...
function deleteUser(userId) {
	if(confirm("Are you sure you want to delete this record?")){
		...
		map["id"]=userId;
		$.ajax({
				url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/user/deleteUser',
				type: 'post',
				headers: {
                'token': localStorage.getItem("token"),
				},
...
function editRole(obj){	
	...
	map["id"]=$(obj).attr("data-id");
	$.ajax({
			url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/role/getByIdRole',
			type: 'post',
			headers: {
                'token': localStorage.getItem("token"),
			},
...
function editUser(userId) {
		...
		map["userId"]=userId;
		$.ajax({
			url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/user/getById',
			type: 'post',
			headers: {
                'token': localStorage.getItem("token"),
			},
...
$('#btnUFSubmit').click(function (event) {
	...
	$.ajax({
			url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/user/'+suffixurl,
			type: 'post',
			headers: {
                'token': localStorage.getItem("token"),
			},
...
$('#btnRolesSubmit').click(function (event) {
	...
	$.ajax({
			url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/role/createRole',
			type: 'post',
			headers: {
                'token': localStorage.getItem("token"),
			},
...
$('#btnsignin').click(function (event) {
	...
	localStorage.setItem("o2crusher.code",$("#code").val().toUpperCase());
	$(this).html("<i class='fa fa-spinner fa-spin'></i>&nbsp;&nbsp;Please Wait...").attr("disabled",true);
	var map={};
	map["mobile"]=$("#mobile").val();
	map["password"]=$("#pass").val();
	$.ajax({
			url: SERVER_URL + localStorage.getItem("o2crusher.code") +'/admin/web/user/login',
			type: 'post',
			data:JSON.stringify(map),
			complete: function (response) {
				if (response.status == 200) {
